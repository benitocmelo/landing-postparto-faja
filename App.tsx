...
const TESTIMONIAL_VIDEOS = [
  ...
  "https://erxxuotslhjluwrlxmyx.supabaseco/storage/v1/object/sign/LANDING%20POST%20PARTO/VSL_POST_PARTO.mp4?token=eyJraWQiOiJzdG9yYWdlLXVybC1zaWduaW5nLWtleV9hZWQxZTBkNS1mNzcwLTRmMDMtODRhYy1jYTk2YzZkZmM1NDQiLCJhbGciOiJIUzI1NiJ9.eyJ1cmwiOiJMQU5ESU5HIFBPU1QgUEFSVE8vVlNMX1BPU1RfUEFSVE8ubXA0IiwiaWF0IjoxNzY4MDg1NDIxLCJleHAiOjE3OTk2MjE0MjF9.qeLRDt57HU_vY6e66-V1fhAq4e0saLazMgzvxEEifpQ" 
];
  ...
  return (
    <div className="pb-24 overflow-x-hidden bg-cream">
      ...
      <section className="px-5 pt-16 pb-16 text-center max-w-lg mx-auto">
        ...
        {/* Tamaño de fuente ajustado y tracking suavizado para evitar que las letras se peguen */}
        <h1 className="text-[1.65rem] xs:text-[1.85rem] sm:text-3xl md:text-5xl font-black leading-[1.25] md:leading-tight mb-8 tracking-tight break-words">
        ...
        <p className="text-gray-500 text-base md:text-lg mb-10 max-w-sm mx-auto leading-relaxed">
          No es grasa rebelde, es una separación muscular. Ciérrala y aplana tu vientre con el método exacto en <b>21 días</b>.
        </p>

        <div ref={vslRef} className="relative aspect-video bg-black rounded-[2.5rem] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.3)] overflow-hidden mb-12 border border-white/10 group">
      ...
      <section className="px-5 py-24 bg-white rounded-t-[3.5rem] shadow-2xl w-full max-w-2xl mx-auto">
        <div className="text-center mb-16 px-2">
          <span className="text-lime-600 text-[10px] font-black tracking-widest uppercase mb-4 block">BIOMECÁNICA ABDOMINAL</span>
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-black leading-tight tracking-tight text-sage-900">EL SECRETO DE <br/><span className="text-red-600 underline decoration-red-200 underline-offset-8">TU FAJA NATURAL</span></h2>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
           <div className="group">
              <div className="relative rounded-[2.5rem] overflow-hidden mb-6 border-2 border-red-50 shadow-sm transition-transform group-hover:scale-[1.02]">
                 ...
                 <div className="absolute bottom-6 left-6 right-6 bg-white/90 backdrop-blur-md p-4 rounded-2xl shadow-xl">
                    <p className="text-red-600 font-black text-center text-xs uppercase tracking-widest">EL EFECTO GLOBO</p>
                 </div>
              </div>
              <h3 className="text-xl font-bold text-sage-900 mb-3 text-center">Ejercicios Tradicionales</h3>
              <p className="text-gray-500 text-sm leading-relaxed text-center px-4">Abren tu diástasis y empujan el útero hacia abajo, debilitando tu suelo pélvico.</p>
           </div>
           
           <div className="group">
              <div className="relative rounded-[2.5rem] overflow-hidden mb-6 border-4 border-lime-400 shadow-[0_20px_40px_-10px_rgba(132,204,22,0.2)] transition-transform group-hover:scale-[1.02]">
                 ...
                 <div className="absolute bottom-6 left-6 right-6 bg-lime-400 p-4 rounded-2xl shadow-xl">
                    <p className="text-sage-900 font-black text-center text-xs uppercase tracking-widest">EL EFECTO CORSET</p>
                 </div>
              </div>
              <h3 className="text-xl font-bold text-sage-900 mb-3 text-center">Técnica Hipopresiva</h3>
              <p className="text-gray-500 text-sm leading-relaxed text-center px-4">Succiona y une las paredes abdominales, cerrando el hueco y aplanando el vientre.</p>
           </div>
        </div>
      </section>
      ...
      <section className="py-24 bg-cream px-5">
        <div className="max-w-md mx-auto">
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-black text-center mb-16 leading-tight tracking-tight">
            Activa tu <span className="text-lime-600 italic">"Corset Invisible"</span>
          </h2>
          
          <div className="relative rounded-[3.5rem] overflow-hidden shadow-2xl border-4 border-white mb-12 group">
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-sage-900/60 to-transparent flex flex-col justify-end p-8">
              <p className="text-lime-400 text-[10px] font-black uppercase tracking-[0.3em] mb-2">Internal Scan Active</p>
              <p className="text-white text-sm font-bold opacity-80">Re-programación del Transverso Abdominal</p>
            </div>
          </div>
          ...
          <div className="space-y-12">
            {[
              ...
              <div key={i} className="flex gap-6 items-start">
                <div className="w-14 h-14 rounded-[1.2rem] bg-white shadow-lg flex items-center justify-center text-lime-600 shrink-0 border border-sage-50">
                  <i className={`fa-solid ${step.icon} text-xl`}></i>
                </div>
                <div>
                  <h3 className="font-bold text-xl text-sage-900 mb-1">{step.title}</h3>
      ...
      <section className="py-24 bg-white overflow-hidden">
        <h2 className="text-2xl sm:text-3xl font-black px-5 mb-12 text-center tracking-tight">CASOS DE ÉXITO VERIFICADOS</h2>
        <Carousel autoPlay={true}>
          {TESTIMONIAL_IMAGES.map((src, i) => (
            <div key={i} className="shrink-0 w-[85vw] max-w-[340px] snap-center">
              <div className="relative rounded-[3rem] overflow-hidden border-[8px] border-cream shadow-2xl bg-white aspect-[4/5] flex items-center justify-center">
                 <img src={src} alt={`Testimonio Verificado ${i+1}`} className="w-full h-full object-contain bg-white" />
                 <div className="absolute top-4 right-4 bg-lime-400 text-sage-900 text-[9px] font-black px-4 py-2 rounded-full shadow-lg border-2 border-white uppercase tracking-tighter">CASO REAL</div>
              </div>
            </div>
          ))}
        </Carousel>
      ...
      {/* COMUNIDAD (VIDEOS) */}
      <section className="py-24 bg-sage-900 text-white rounded-[3.5rem] mx-2 shadow-inner overflow-hidden">
        <div className="px-8 mb-12 text-center">
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-black leading-tight tracking-tight mb-4">NUESTRA <br/><span className="animate-gradient bg-gradient-to-r from-lime-500 via-lime-200 to-lime-500 bg-clip-text text-transparent">COMUNIDAD VIP</span></h2>
            <div className="flex justify-center gap-1">
                {[1,2,3,4,5].map(i => <Star key={i} size={14} className="fill-lime-400 text-lime-400" />)}
            </div>
        </div>
        <Carousel autoPlay={false}>
          {TESTIMONIAL_VIDEOS.map((src, i) => (
            <div key={i} className="shrink-0 w-[260px] snap-center aspect-[9/16] bg-black rounded-[2.5rem] overflow-hidden shadow-2xl relative border border-white/5">
              <video className="w-full h-full object-cover" controls preload="metadata">
                <source src={src} type="video/mp4" />
              </video>
              <div className="absolute top-5 left-5 pointer-events-none">
                 <div className="bg-white/10 backdrop-blur-xl rounded-full px-4 py-2 text-[9px] font-black border border-white/20 tracking-widest uppercase">MAMÁ #0{i+1}</div>
              </div>
            </div>
          ))}
        </Carousel>
      ...
      <section id="oferta" className="px-5 py-24">
        <div className="relative max-w-lg mx-auto bg-[#0a1412] rounded-[3.5rem] p-8 sm:p-10 text-white overflow-hidden shadow-[0_40px_100px_-20px_rgba(132,204,22,0.15)] border border-white/5">
          <div className="absolute top-0 right-0 left-0 bg-gradient-to-r from-red-600 to-orange-600 text-white font-black text-[10px] py-4 text-center shadow-lg animate-pulse flex items-center justify-center gap-2 z-10 tracking-[0.2em]">
             <Clock size={12} fill="white" /> 12 CUPOS CON PRECIO PROMOCIONAL
          </div>
          
          <div className="mt-12 mb-10 text-center relative z-20">
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-black mb-6 tracking-tight uppercase leading-none">RETO POSPARTO <br/><span className="animate-gradient bg-gradient-to-r from-lime-400 via-white to-lime-400 bg-clip-text text-transparent italic tracking-normal">21 DÍAS</span></h2>
            
            <div className="bg-white/5 backdrop-blur-2xl rounded-[2.5rem] p-6 sm:p-8 border border-white/10 mb-10 text-left">
                <p className="text-[10px] font-bold text-lime-400 tracking-[0.3em] mb-6 uppercase">Tu Paquete Incluye:</p>
                ...
                <div className="border-t border-white/10 pt-8 text-center">
                    <p className="text-gray-500 line-through text-sm mb-1 tracking-widest font-black uppercase opacity-70">VALOR REAL: $197.00 USD</p>
                    <div className="flex flex-col items-center justify-center mb-8">
                        <span className="text-5xl sm:text-6xl font-black text-white tracking-tighter animate-gradient bg-gradient-to-r from-white via-lime-400 to-white bg-clip-text text-transparent">$69.99</span>
                        <span className="text-[10px] text-lime-400 font-black tracking-[0.5em] mt-2">DÓLARES AMERICANOS</span>
                    </div>
                </div>

                <a href="https://go.hotmart.com/D102309348Q?ap=4cfb" className="group block w-full bg-gradient-to-r from-lime-400 via-lime-300 to-lime-500 text-sage-900 font-black py-6 rounded-3xl text-lg sm:text-xl shadow-[0_15px_40px_rgba(132,204,22,0.3)] hover:scale-[1.03] active:scale-95 transition-all text-center relative overflow-hidden">
                  EMPEZAR RETO AHORA
                </a>
            </div>
            ...
            <div className="flex flex-col items-center gap-6">
                <div className="flex justify-center gap-4 text-3xl sm:text-4xl">
                    <div className="bg-white p-2 rounded-xl shadow-[0_5px_15px_rgba(255,255,255,0.1)]"><i className="fa-brands fa-cc-visa text-[#1A1F71]"></i></div>
                    <div className="bg-white p-2 rounded-xl shadow-[0_5px_15px_rgba(255,255,255,0.1)]"><i className="fa-brands fa-cc-mastercard text-[#EB001B]"></i></div>
                    <div className="bg-white p-2 rounded-xl shadow-[0_5px_15px_rgba(255,255,255,0.1)]"><i className="fa-brands fa-cc-paypal text-[#003087]"></i></div>
                    <div className="bg-white p-2 rounded-xl shadow-[0_5px_15px_rgba(255,255,255,0.1)]"><i className="fa-brands fa-cc-amex text-[#006FCF]"></i></div>
                </div>
                <div className="flex items-center gap-2 text-white/40 text-[9px] font-bold tracking-widest uppercase">
                    <Lock size={12} /> Pago Seguro Hotmart® Cifrado 256-bit
                </div>
            </div>
          </div>
        </div>
      ...
      <section className="px-5 py-24 max-w-md mx-auto">
        <div className="bg-white rounded-[3.5rem] overflow-hidden shadow-2xl border border-sage-50 text-center pb-12">
          <div className="relative">
              ...
              <div className="absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-1/2 flex gap-3">
                 <div className="bg-sage-900 text-white p-4 rounded-3xl shadow-2xl border-4 border-white">
          ...
          <div className="px-8 sm:px-10 mt-12">
            <Badge text="TU MENTORA EXPERTA" />
            <h2 className="text-3xl sm:text-4xl font-black mb-6 tracking-tight">NATALIA ALARCÓN</h2>
            
            <div className="grid grid-cols-1 gap-4 text-left mb-8">
                <div className="flex items-center gap-3 bg-sage-50 p-4 rounded-2xl border border-sage-100/50">
                    <Award size={24} className="text-lime-600 shrink-0" />
                    <span className="text-xs font-bold text-sage-800">Especialista Certificada en Hipopresivos y Diástasis.</span>
                </div>
                <div className="flex items-center gap-3 bg-sage-50 p-4 rounded-2xl border border-sage-100/50">
                    <Users size={24} className="text-lime-600 shrink-0" />
                    <span className="text-xs font-bold text-sage-800">+5,000 Alumnas Transformadas con éxito.</span>
                </div>
                <div className="flex items-center gap-3 bg-sage-50 p-4 rounded-2xl border border-sage-100/50">
                    <Heart size={24} className="text-lime-600 shrink-0" />
                    <span className="text-xs font-bold text-sage-800">Referente en Salud Materna y Fitness Postparto.</span>
                </div>
            </div>

            <p className="text-gray-500 text-sm leading-relaxed italic px-2 font-medium">"Mi propósito es devolverte la seguridad que el embarazo te quitó, con un método respetuoso y altamente efectivo."</p>
          </div>
        </div>
      </section>

      {/* FOOTER */}
      <footer className="py-24 px-5 bg-sage-900 text-white/30 text-center text-[10px] rounded-t-[3.5rem]">
        <div className="flex justify-center gap-10 mb-12 text-xs text-white/70 font-black tracking-widest">
      ...
      {view === 'landing' ? <LandingPage onNavigate={setView} /> : (
        <div className="p-10 max-w-2xl mx-auto min-h-screen flex flex-col justify-center">
          <button onClick={() => setView('landing')} className="mb-16 text-sage-900 font-black flex items-center gap-4 group">
            <div className="w-12 h-12 rounded-2xl bg-sage-100 flex items-center justify-center group-hover:bg-lime-400 transition-all shadow-sm">
                <ArrowLeft size={18} strokeWidth={3}/>
            </div>
            VOLVER AL RETO
          </button>
          <h1 className="text-4xl font-black mb-10 tracking-tight uppercase">{view === 'privacy' ? 'Privacidad' : 'Términos'}</h1>
